  ...
  if (itemIndex < self.imageUrls.count) {
    ...
    if ([urlString isKindOfClass:[UIImage class]]) {
     ...
     // [cell.imageView setImageWithURLString:urlString placeholder:self.placeholder];
        ...
        [cell.imageView sd_setImageWithURL:[NSURL URLWithString:[[NSString stringWithFormat:@"%@%@",imageHostURL,urlString]stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] placeholderImage:[UIImage imageNamed:@"shibai.png"] options:SDWebImageRetryFailed];
//        cell.imageView.contentMode = UIViewContentModeScaleToFill;
//        cell.imageView.image = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",imageHostURL,urlString]]]];
