...
static void on_message(mopush *mosq, void *obj, struct mopush_message *mosq_msg)
{
    ...
    @autoreleasepool {
        NSString *topic = [NSString stringWithUTF8String: mosq_msg->circleName];
...
- (MQTTClient*) initWithClientId: (NSString*) clientId withAppName:(NSString *)appName
{
    mopush_set_appName(appName.UTF8String);
                    ...
                    cleanSession: (BOOL )cleanSession
{
    if ((self = [super init])) {
        ...
        mopush_message_callback_set(mosq, on_message);
...
//    mosquitto_subscribe(mosq, &mid, cstrTopic, qos);
    mopush_subscribe(mosq, &mid, cstrTopic, qos);
    if (completionHandler) {
...
//    mosquitto_unsubscribe(mosq, &mid, cstrTopic);
    mopush_unsubscribe(mosq, &mid, cstrTopic);
    if (completionHandler) {
